...
ArkInventory.IsCataclysm = ( ( select( 4, GetBuildInfo( ) ) or 0 ) >= 40000 )
...
if not ArkInventory.IsCataclysm then
	
	ArkInventory.Const.Category.Code.System[413] = {
		["id"] = "SYSTEM_SOULSHARD",
		["text"] = ArkInventory.Localise["CATEGORY_SYSTEM_SOULSHARD"],
	}
	ArkInventory.Const.Category.Code.System[410] = {
		["id"] = "SYSTEM_PROJECTILE",
		["text"] = ArkInventory.Localise["WOW_ITEM_TYPE_PROJECTILE"],
	}
	
	ArkInventory.Const.Category.Code.Empty[304] = {
		["id"] = "EMPTY_SOULSHARD",
		["text"] = ArkInventory.Localise["CATEGORY_SYSTEM_SOULSHARD"],
	}
	ArkInventory.Const.Category.Code.Empty[310] = {
		["id"] = "EMPTY_PROJECTILE_BULLET",
		["text"] = ArkInventory.Localise["WOW_ITEM_TYPE_PROJECTILE_BULLET"],
	}
	ArkInventory.Const.Category.Code.Empty[311] = {
		["id"] = "EMPTY_PROJECTILE_ARROW",
		["text"] = ArkInventory.Localise["WOW_ITEM_TYPE_PROJECTILE_ARROW"],
	}

	
	ArkInventory.Const.Slot.Data[ArkInventory.Const.Slot.Type.Soulshard] = {
	...
	ArkInventory.Const.Slot.Data[ArkInventory.Const.Slot.Type.Bullet] = {
	...
	ArkInventory.Const.Slot.Data[ArkInventory.Const.Slot.Type.Arrow] = {
...
ArkInventory.Const.DatabaseDefaults.global = {
	["option"] = {
		...
		["ldb"] = {
			["mounts"] = {
				["ground"] = {
					["corrected"] = { }, -- [spellid] = -1 (is not ground) | 0 (is ground)
				},
				["flying"] = {
					["corrected"] = { }, -- [spellid] = -1 (is not flying) | 0 (is flying)
				},
				["water"] = {
					["corrected"] = { }, -- [spellid] = -1 (is not watr) | 0 (is water)
				},
			},
		},
	},
...
function ArkInventory.OnInitialize( )
	...
	--ArkInventory.Global.Location[ArkInventory.Const.Location.Token].bagCount = 1
...
function ArkInventory.OnEnable( )
	...
	ArkInventory:RegisterEvent( "COMPANION_UPDATE", "LISTEN_COMPANION_UPDATE" )
...
function ArkInventory.ItemCategoryGetDefaultActual( i )
	...
	if ( not ArkInventory.IsCataclysm ) and ( itemEquipLoc == "INVTYPE_AMMO" or itemType == ArkInventory.Localise["WOW_ITEM_TYPE_PROJECTILE"] ) then
	...
	-- soul shards
	if ( not ArkInventory.IsCataclysm ) and ( ArkInventory.ObjectStringDecodeItem( i.h ) == 6265 or ArkInventory.PT_ItemInSets( i.h, ArkInventory.Localise["PT_CATEGORY_SOULSHARD"] ) ) then
...
function ArkInventory.ItemCategoryGetDefaultEmpty( loc_id, bag_id )
	...
	if not ArkInventory.IsCataclysm and bt == ArkInventory.Const.Slot.Type.Arrow then
		if clump then
	...
	if not ArkInventory.IsCataclysm and bt == ArkInventory.Const.Slot.Type.Bullet then
		if clump then
	...
	if not ArkInventory.IsCataclysm and bt == ArkInventory.Const.Slot.Type.Soulshard then
		if clump then
			return ArkInventory.CategoryGetSystemID( "SYSTEM_SOULSHARD" )
		else
...
function ArkInventory.Frame_Main_Anchor_Set( loc_id, rescale )
	...
	local t = ( ArkInventory.LocationOptionGet( loc_id, "anchor", loc_id, "t" ) or UIParent:GetTop( ) ) * ( rescale or 1 )
	local b = ( ArkInventory.LocationOptionGet( loc_id, "anchor", loc_id, "b" ) or UIParent:GetBottom( ) ) * ( rescale or 1 )
	local l = ( ArkInventory.LocationOptionGet( loc_id, "anchor", loc_id, "l" ) or UIParent:GetLeft( ) ) * ( rescale or 1 )
	local r = ( ArkInventory.LocationOptionGet( loc_id, "anchor", loc_id, "r" ) or UIParent:GetRight( ) ) * ( rescale or 1 )
			...
			for b = 1, bpr do
				if Layout.bar[j+b-1] then
					if Layout.bar[j+b-1].count > 0 then
						...
						bf = bf + 1
						bc = bc + 1
						--table.insert( vr, Layout.bar[j+b-1] )
						table.insert( vr, j+b-1 )
					else
...
function ArkInventory.Frame_Item_OnEnter( frame )
	...
	if ArkInventory.Global.Mode.Edit or ArkInventory.Global.Location[loc_id].isOffline or blizzard_id == BANK_CONTAINER or blizzard_id == KEYRING_CONTAINER or loc_id == ArkInventory.Const.Location.Vault or loc_id == ArkInventory.Const.Location.Wearing or loc_id == ArkInventory.Const.Location.Mail or loc_id == ArkInventory.Const.Location.Pet or loc_id == ArkInventory.Const.Location.Mount or loc_id == ArkInventory.Const.Location.Token then
...
function ArkInventory.LocationOptionGetReal( loc_id, ... )
	...
	assert( loc_id ~= nil, "bad author: location id is nil" )
	
	local p = ArkInventory.db.profile.option.location[loc_id]
	local s = string.format( "db.profile.option.location.%s", loc_id )
	
	local n = select( '#', ... )
	assert( n > 0, "bad author: not enough parameters" )
	
	for i = 1, n do
		
		assert( p ~= nil, string.format( "bad author: %s is nil - please reload ui to reset the database or report to author if it continues", s ) )
		
		local k = select( i, ... )
		assert( k ~= nil, string.format( "bad author: parameter %s is nil", i ) )
...
function ArkInventory.LocationOptionSetReal( loc_id, ... )
	...
	local n = select( '#', ... )
	assert( n > 1, "bad author: not enough parameters" )
	
	for i = 1, n - 2 do
		
		assert( p ~= nil, string.format( "bad author: %s is nil - please reload ui to reset database or report to author if it continues", s ) )
	...
	local k = select( n - 1, ... )
	assert( k ~= nil, string.format( "bad author: parameter %s is nil", n - 1 ) )
