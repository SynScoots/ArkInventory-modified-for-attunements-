...
--to do - shrink table - Use info[#info] to get the leaf node name, info[#info-1] for the parent
		...
		ldb = {
			...
			args = {
				currency = {
					order = 1100,
					type = "group",
					name = string.format( "%s: %s", ArkInventory.Localise["LDB_TRACKING"], ArkInventory.Localise["LDB_TRACKING_CURRENCY"] ),
					args = {
						size = {
							order = 100,
							name = ArkInventory.Localise["HEIGHT"],
							...
							isPercent = false,
							get = function( info )
								return ArkInventory.db.char.option.ldb.tracking.currency.size or 0
							end,
							set = function( info, v )
								if v < 0 then v = 0 end
								if v > 48 then v = 48 end
								if ArkInventory.db.char.option.ldb.tracking.currency.size ~= v then
									ArkInventory.db.char.option.ldb.tracking.currency.size = v
				...
				item = {
					order = 1200,
					type = "group",
					name = string.format( "%s: %s", ArkInventory.Localise["LDB_TRACKING"], ArkInventory.Localise["LDB_TRACKING_ITEM"] ),
					args = {
						size = {
							order = 100,
							name = ArkInventory.Localise["HEIGHT"],
							...
							isPercent = false,
							get = function( info )
								return ArkInventory.db.char.option.ldb.tracking.item.size or 0
							end,
							set = function( info, v )
								if v < 0 then v = 0 end
								if v > 48 then v = 48 end
								if ArkInventory.db.char.option.ldb.tracking.item.size ~= v then
									ArkInventory.db.char.option.ldb.tracking.item.size = v
									ArkInventory.LDB.Tracking_Item:Update( )
								end
							end,
						},
...
function ArkInventory.ConfigInternalControlSettings( path )
	...
	local args1 = {
		...
		anchor = {
			...
			get = function( info )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.LocationOptionGet( loc_id, "anchor", loc_id, "point" )
			end,
			set = function( info, v )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				if ArkInventory.LocationOptionGet( loc_id, "anchor", loc_id, "point" ) ~= v then
					ArkInventory.LocationOptionSet( loc_id, "anchor", loc_id, "point", v )
		...
		reposition = {
			order = 550,
			name = ArkInventory.Localise["CONFIG_CONTROL_REPOSITION"],
			desc = function( info )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				return string.format( ArkInventory.Localise["CONFIG_CONTROL_REPOSITION_TEXT"], ArkInventory.Global.Location[loc_id].Name )
			end,
			type = "execute",
			func = function( info )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.LocationOptionSet( loc_id, "anchor", loc_id, "t", nil )
				ArkInventory.LocationOptionSet( loc_id, "anchor", loc_id, "b", nil )
				ArkInventory.LocationOptionSet( loc_id, "anchor", loc_id, "l", nil )
				ArkInventory.LocationOptionSet( loc_id, "anchor", loc_id, "r", nil )
				ArkInventory.Frame_Main_Anchor_Set( loc_id )
			end,
		},
		locked = {
			...
			get = function( info )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.LocationOptionGet( loc_id, "anchor", loc_id, "locked" )
			end,
			set = function( info, v )
				local loc_id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.LocationOptionSet( loc_id, "anchor", loc_id, "locked", v )
				ArkInventory.Frame_Main_Anchor_Set( loc_id )
			end,
		},
		settings = {
...
function ArkInventory.Frame_Config_Hide( )
	ArkInventory.Lib.Dialog:Close( ArkInventory.Const.Frame.Config.Internal )
end
...
function ArkInventory.Frame_Config_Show( ... )
	...
	ArkInventory.Lib.Dialog:Open( ArkInventory.Const.Frame.Config.Internal, ... )
	
end

function ArkInventory.Frame_Config_Toggle( )

--	if xxxxxxxxx:IsVisible( ) then
--		ArkInventory.Frame_Config_Hide( )
--	else
		ArkInventory.Frame_Config_Show( )
--	end
